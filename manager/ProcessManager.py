...
## Copyright (c) 2017, The Sumokoin Project (www.sumokoin.org)
'''
Process managers for recoald, recoal-wallet-cli and recoal-wallet-rpc
'''
...
class WalletRPCManager(ProcessManager):
    def __init__(self, resources_path, wallet_file_path, wallet_password, app, log_level=2):
        ...
        wallet_log_path = os.path.join(os.path.dirname(wallet_file_path), "recoal-wallet-rpc.log")
        resources_path = resources_path.replace(' ' ,'__SPACE_REPLACE__')
        wallet_file_path  = wallet_file_path.replace(' ' ,'__SPACE_REPLACE__')
        wallet_log_path = wallet_log_path.replace(' ' ,'__SPACE_REPLACE__')
        wallet_rpc_args = u'%s/bin/recoal-wallet-rpc --disable-rpc-login --wallet-file %s --log-file %s --rpc-bind-port %d --log-level %d --daemon-port %d --password %s' % (resources_path, wallet_file_path, wallet_log_path, RPC_DAEMON_PORT+2, log_level, RPC_DAEMON_PORT, wallet_password)

        ProcessManager.__init__(self, wallet_rpc_args, "recoal-wallet-rpc")
